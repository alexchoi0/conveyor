...
impl SidecarConfig {
    pub fn from_env() -> Result<Self> {
        let pod_name = std::env::var("ETL_POD_NAME")
            .unwrap_or_else(|_| hostname());

        let namespace = std::env::var("ETL_NAMESPACE")
            .unwrap_or_else(|_| "default".to_string());

        let node_name = std::env::var("ETL_NODE_NAME").ok();

        let local_ports = parse_ports(
            &std::env::var("ETL_LOCAL_PORTS").unwrap_or_default()
        );

        let cluster_endpoint = std::env::var("ETL_CLUSTER_ENDPOINT")
            .context("ETL_CLUSTER_ENDPOINT must be set")?;

        let listen_addr: SocketAddr = std::env::var("ETL_SIDECAR_LISTEN")
            .unwrap_or_else(|_| "0.0.0.0:50053".to_string())
            .parse()
            .context("Invalid ETL_SIDECAR_LISTEN address")?;

        let pod_ip = std::env::var("ETL_POD_IP")
