...
use etl_proto::sidecar::sidecar_data_plane_server::SidecarDataPlaneServer;

use etl_sidecar::config::SidecarConfig;
use etl_sidecar::discovery::GrpcReflectionDiscovery;
use etl_sidecar::routing::{RoutingTable, LocalRouter, RemoteRouter};
use etl_sidecar::cluster_client::{ClusterRegistration, HeartbeatLoop};
use etl_sidecar::SidecarDataPlaneImpl;

#[tokio::main]
async fn main() -> Result<()> {
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| "etl_sidecar=info,tower_http=debug".into()))
