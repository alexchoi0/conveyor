...
use clap::Args;
use etl_dsl::manifest::{AnyManifest, ResourceKind};
...
fn load_manifest_file(path: &PathBuf) -> Result<AnyManifest> {
    ...
    let manifest = match kind_probe.kind {
        ResourceKind::Source => {
            let m: etl_dsl::SourceManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Source(m)
        }
        ResourceKind::Transform => {
            let m: etl_dsl::TransformManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Transform(m)
        }
        ResourceKind::Sink => {
            let m: etl_dsl::SinkManifest = serde_yaml::from_str(&content)?;
            AnyManifest::Sink(m)
        }
        ResourceKind::Pipeline => {
            let m: etl_dsl::LegacyPipelineManifest = serde_yaml::from_str(&content)?;
