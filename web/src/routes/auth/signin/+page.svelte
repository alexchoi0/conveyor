...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
  <div class="w-full max-w-md px-4">
    <Card class="border-0 shadow-xl">
      <CardHeader class="space-y-4 text-center pb-2">
        <div class="flex justify-center">
          <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-primary shadow-lg">
            <GitBranch class="h-7 w-7 text-primary-foreground" />
          </div>
        </div>
        <div class="space-y-2">
          <CardTitle class="text-2xl font-bold tracking-tight">Welcome back</CardTitle>
          <CardDescription class="text-base">Sign in to ETL Router Dashboard</CardDescription>
        </div>
      </CardHeader>
      ...
      <CardContent class="space-y-4 pt-4">
        ...
        <div class="grid gap-3">
          <Button
            variant="outline"
            class="h-11 w-full justify-center gap-3 font-medium"
    ...
    <p class="mt-6 text-center text-sm text-muted-foreground">
      ETL Router Dashboard &copy; {new Date().getFullYear()}
    </p>
  </div>
</div>
